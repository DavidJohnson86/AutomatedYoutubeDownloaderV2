...
setup(name='Youtube_Downloader',
      ...
      options={"build_exe": {"includes":["YotubeDownloader","easygui","Search","urllib","random"],
      'include_files':[
            os.path.join(PYTHON_INSTALL_DIR, 'DLLs', 'tk86t.dll'),
            os.path.join(PYTHON_INSTALL_DIR, 'DLLs', 'tcl86t.dll'),
			os.path.dirname(os.path.realpath(__file__))+'\\Data'],"packages": ["urllib","tkinter", "os","cx_Freeze"]}},
      executables = [Executable("graphical_interface.py")])

# COMMAND C:\Python36\Python.exe setup.py build bdist_msi